...
class MessageService {
  ...
  static async getTotalCountByChatId(id: string) {
    ...
    return count.rows.length === 1 ? Number(count.rows[0].count) : null;
  }
  ...
  static async getMessagesByChatId(
    ...
    const messages = messagesRes.rows.map((message) => {
      const newMessage = {
        ...
        image: message.image
          ? `${C.API_DOMAIN}/images/${message.id}.png`
          : false,
      };
  ...
  static async sendTextMessage({
    ...
    try {
      const newMessage = await db.query(
        `INSERT INTO Messages (chat_id, createdon, username, message, image) 
              VALUES ($1, $2, $3, $4, $5) RETURNING id`,
        [chatId, createdon, userName, message, Boolean(image)]
      );
      ...
      const imageUrl = image
        ? `;${C.API_DOMAIN}/images/${lastMessageId}.png`
        : false;
